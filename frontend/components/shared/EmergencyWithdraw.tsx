...
export default function EmergencyWithdraw() {
  ...
  const handleEmergencyWithdraw = async () => {
    ...
    try {
      ...
      setStatus("error");
      setError(e?.message || "Error during emergency withdrawal");
      setTxHash(undefined);
    }
  };
    ...
    <div className="w-full max-w-sm mx-auto p-4 rounded-xl bg-red-900/20 border border-red-500/50 shadow-lg">
      ...
      <div className="flex flex-col gap-3">
        ...
        <input
          ...
          className="bg-gray-900 text-white border border-red-500/50 rounded-lg px-3 py-2 focus:outline-none focus:border-red-400 transition font-mono"
        />

        <button
          onClick={handleEmergencyWithdraw}
          disabled={
            !address || !shares || isConfirming || status === "withdrawing"
          }
          className="bg-red-600 hover:bg-red-700 text-white font-bold py-2 rounded-xl transition disabled:opacity-50"
        >
          {status === "withdrawing"
